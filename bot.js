// multi_bot_fleet_with_surnames.js
// Spawns multiple Mineflayer bots (default 30) sequentially (~15s apart) with Vietnamese-style names (surname+given).
// Each bot registers/logins with /register /login /spawn, stands still before commands, and performs simple tasks.
// WARNING: Use only on servers you own or are allowed to test bots on.
...
// Updated generateNames function (Vietnamese names with surname + optional middle + given)
function generateNames(count) {
  const SURNAMES = ['nguyen','tran','le','pham','hoang','phan','vu','vo','dang','bui','do','duong','ngo','dinh','ly','truong','hoang2','phamle','ho'];
...
function createBotWithName(username) {
  ...
  bot.once('spawn', async () => {
    ...
    async function stopMovement(ms = 120) {
      ...
      const controls = ['forward','back','left','right','sprint','jump','sneak']
...
async function main(){
  ...
  for(let i=0;i<names.length;i++){
    setTimeout(()=>{
      console.log(`Spawn bot ${i+1}: ${names[i]}`)
      createBotWithName(names[i])
    }, i*JOIN_INTERVAL_MS)
  }
}
main()
